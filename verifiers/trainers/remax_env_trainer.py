...
class ReMaxEnvTrainer(GRPOTrainer):
    ...
    def _generate_and_score_completions(
        ...
        if self.accelerator.is_main_process:
            ...
            completion_messages_baseline = env_result_baseline['messages']
            completion_mask_baseline = env_result_baseline['mask']
        else:
            completion_ids_baseline = [None] * len(all_prompts)
            completion_messages_baseline = [None] * len(all_prompts)
            completion_mask_baseline = [None] * len(all_prompts)

        completion_ids_baseline = broadcast_object_list(completion_ids_baseline, from_process=0)
        completion_messages_baseline = broadcast_object_list(completion_messages_baseline, from_process=0)
        completion_mask_baseline = broadcast_object_list(completion_mask_baseline, from_process=0)
        ...
        completion_ids_baseline = completion_ids_baseline[process_slice]
        completion_messages_baseline = completion_messages_baseline[process_slice]
        completion_mask_baseline = completion_mask_baseline[process_slice]
        ...
        completions_baseline = completion_messages_baseline
        rewards_per_func = torch.zeros(len(prompts), len(self.reward_funcs), device=device)
        rewards_per_func_baseline = torch.zeros(len(prompts), len(self.reward_funcs), device=device)
